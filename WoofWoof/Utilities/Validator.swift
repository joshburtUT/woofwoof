...
struct PasswordValidator: ValidatorConvertible {
    func validated(_ value: String) throws -> String {
        guard !value.isEmpty else { throw ValidationError("Password must have at least 6 characters, with at least one character and one numeric character.") }
        guard value.count >= 6 else { throw ValidationError("Password must have at least 6 characters, with at least one character and one numeric character.") }
